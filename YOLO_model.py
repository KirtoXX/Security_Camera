...
class Detection_api:
    ...
    def __init__(self):
        ...
        self.weights_file = 'yolo/weights/YOLO_tiny.ckpt'
        ...
        #-------绘制图片-------------------
        self.tofile_img = 'yolo/test/output.jpg'
    ...
    #解析网络的预测结果
    def decode_inference(self, output):
    ...
    def inference(self, img):
        ...
        net_output = self.sess.run(self.fc_19, feed_dict=in_dict)    #通过tf.sess转换成 numpy 格式
        return net_output

    #绘图
    def rectangle(self, img, results):
        img_cp = img.copy()
    ...
    def predict(self, filename):
        ...
        net_output = self.inference(img)
        result = self.decode_inference(net_output[0])
        img_cp = self.rectangle(img, result)
...
def main():
    yolo = Detection_api()
    yolo.build_networks()
    #yolo.predict('Persion_image/2017-09-07-164456.jpg')
    yolo.predict('test_image/003.jpg')
