...
class SsdMetaArchTest(tf.test.TestCase):
    ...
    for input_shape in input_shapes:
      ...
      with tf_graph.as_default():
        preprocessed_input_placeholder = tf.placeholder(tf.float32,
                                                        shape=input_shape)
        prediction_dict = self._model.detectFaces(preprocessed_input_placeholder)
  ...
  def test_loss_results_are_correct(self):
    ...
    prediction_dict = self._model.detectFaces(preprocessed_input)
  ...
  def test_restore_map_for_classification_ckpt(self):
    ...
    with test_graph_detection.as_default():
      ...
      prediction_dict = self._model.detectFaces(preprocessed_inputs)
